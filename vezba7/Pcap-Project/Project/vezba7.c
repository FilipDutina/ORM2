...
int main()
{
	...
	char filter_exp[] = "ether host 38:d5:47:de:eb:22 and (ip and(udp or tcp))"; /*ovde zameni enter host, to odradis tako sto u terminalu ukucas getmac*/
...
/*odavde pa nadalje se koristi druga polovina za 5-ti i ostala 6 zadataka*/
...
/*trebalo bi da je ovo 8-mi zadatak*/
...
void print_tcp_header(tcp_header * th) {
	...
	printf("\n=============================================================");
	printf("\n\tAPPLICATION LAYER   -  Transmison Control Protocol (TCP)");

	printf("\n\tSource port:\t\t%u", ntohs(th->dest_port));
	printf("\n=============================================================");

	return;
}

/*trebalo bi da je ovo 11-ti zadatak*/
void print_application_data(unsigned char* data, long data_length) {
	printf("\n=============================================================");
	printf("\n\tDATA   -  Application Data\n");
	printf("\n\tLENGTH: %dB", data_length);
...
void packet_handler(unsigned char *param, const struct pcap_pkthdr*
	...
	if (ih->next_protocol == 17) {
		...
		print_tcp_header(th);
		app_data = (unsigned char*)th + 8;
		app_length = (packet_header->len - th->header_length - ih->header_length);
	}
	ControlExit++;
	printf("%d\n", ControlExit);
}