...
int main()
{
	...
	for (int i = 0; i < number_of_packets; i++)
	{
		...
		{
			packet_data = setup_header(data_array[i], packet_data, DEFAULT_BUFLEN, i+2);
			id = ((*(packet_data + 51))<<32) + ((*(packet_data + 52)) << 24) + ((*(packet_data + 53)) << 16) + ((*(packet_data + 54)) << 8) + *(packet_data + 55);
			printf("ID: %d ", id);
		...
		{
			packet_data = setup_header(data_array[i], packet_data, size_of_last, i+2);
			id = ((*(packet_data + 51))<<32) + ((*(packet_data + 52)) << 24) + ((*(packet_data + 53)) << 16) + ((*(packet_data + 54)) << 8) + *(packet_data + 55);
			printf("Last ID: %d ", id);
...
void sendFirstTwoPackets(unsigned char* data_ext, unsigned char* packet_data, unsigned char* number_of_elements, int packet_len)
{
	for (int i = 0; i < 2; i++)
	{
		if (i == 0)
		{
			packet_data = setup_header(data_ext, packet_data, strlen(data_ext) + 1, i);
			id = ((*(packet_data + 51)) << 32) + ((*(packet_data + 52)) << 24) + ((*(packet_data + 53)) << 16) + ((*(packet_data + 54)) << 8) + *(packet_data + 55);
			printf("ID: %d ", id);
			if (pcap_sendpacket(device_handle, packet_data, strlen(data_ext) + 1 + TOTAL_HEADER_SIZE) == -1)
			{
		...
		{
			packet_data = setup_header(number_of_elements, packet_data, strlen(number_of_elements) + 1, i);
			id = ((*(packet_data + 51)) << 32) + ((*(packet_data + 52)) << 24) + ((*(packet_data + 53)) << 16) + ((*(packet_data + 54)) << 8) + *(packet_data + 55);
			printf("ID: %d ", id);
