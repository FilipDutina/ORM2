...
float chunk;
int loader;
int tw = 50;	//size of bar
int a;
int myNumOfPackets = 0;
float chunk2;
...
int main()
{
	...
	char filter_exp[] = "ether src c8:0a:a9:68:a1:05 and udp and ip";
	...
	//**************************************************************************************************************************************
	chunk = NMBR_OF_PACKETS / 100;
	chunk2 = (float)NMBR_OF_PACKETS / 100.0;
	printf("chunk2: %f\n", chunk2);
	a = 100 / tw;
	loader = 0;
...
void packet_handler(unsigned char *param, const struct pcap_pkthdr* packet_header, const unsigned char* packet_data)
{
	...
	if (ih->src_addr[0] == 192 && ih->src_addr[1] == 168 && ih->src_addr[2] == 0 && ih->src_addr[3] == 49 && src_port == 8080)
	{
		fwrite(packet_data + 42, 1, (ntohs(uh->datagram_length) - 8), fp);

		//printf("Source: %u.%u.%u.%u,  Packet[%d]: %s, %d byte\r", ih->src_addr[0], ih->src_addr[1], ih->src_addr[2], ih->src_addr[3], i, time_string, packet_header->len);
		//nmbrOfPackets
